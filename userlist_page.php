<?php
	...
	{
		$cust_ID=$_GET['id'];
		mysqli_query($connect,"DELETE FROM user WHERE ID='$cust_ID'");
		?>
		<script>
		alert("Record deleted!");
		location.assign("userlist_page.php");
		</script>
		<?php
	}
?>
	...
	<tr>
		...
		<?php 
			if (!isset($_GET["download"])) 
			{
				...
				<a href="edituser_page.php?edit&id=<?php echo $row['ID'];?>">Edit </a>
				</td>
				
				<td style="border: 1px solid black" >
				<a href="userlist_page.php?delete&id=<?php echo $row['ID'];?>" onclick="return confirmation()"> Delete </a>
				</td>
				<?php 
			}
		?>
